...
exports.create = function(name, chatBot, options) {
	...
	trigger.options.maxlength = trigger.options.maxlength || 300;
	trigger.options.toolongerr = trigger.options.toolongerr || "---Definition too long---";
	return trigger;
};
    ...
    if (term) {
        request({method:'GET',encoding:'utf8',uri:'http://api.urbandictionary.com/v0/define?term='+encodeURI(term),json:true,followAllRedirects:true}, function(error, response, body) {
			if (error) {
				...
				that._sendMessageAfterDelay(toId, ":summerghost:");
				return;
			}
			if (!error && response.statusCode == 200) {
					...
					} else that._sendMessageAfterDelay(toId, ":summerghost:");
            }
        });
        return true;
    }
